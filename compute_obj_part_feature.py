...
def show_anns(anns,return_mask=False):
    if len(anns) == 0:
        return
    img = np.ones((anns.shape[1], anns.shape[2], 4))
    if return_mask:
        img_mask=np.zeros((anns.shape[1], anns.shape[2]))
    img[:,:,3] = 0
...
def main(args):
    ...
    image_dir = os.path.join(base_dir, 'color_segmented')#'images') #
    ...
    for i in trange(len(image_paths)):
        ...
        ann_img,mask_img = show_anns(show_img,return_mask=True)
        save_img_path = obj_feat_path_list[i].replace('.npy', '_mask.png')
        save_mask_path = obj_feat_path_list[i].replace('.npy', '.png')
        Image.fromarray((ann_img * 255).astype(np.uint8)).save(save_img_path)
        Image.fromarray((mask_img).astype(np.uint8)).save(save_mask_path)
