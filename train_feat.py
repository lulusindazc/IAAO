...
import sys
from scene import Scene, GaussianModel, feat_decoder, skip_feat_decoder,skip_feat_class_decoder
...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    num_classes = 256 #dataset.num_classes
    ...
    my_feat_decoder = skip_feat_class_decoder(dataset.distill_feature_dim).cuda()
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        if iteration < opt.update_features_until_iter:
            ...
            # import pdb; pdb.set_trace()
            rendered_feat_bhwc = rendered_feat.unsqueeze(0) #F.interpolate(rendered_feat.unsqueeze(0), size=gt_clip_feat.shape[1:], mode="bilinear", align_corners=False)
            # Decode low dim feature to clip and dino features
            dino_feat, class_feat, part_feat = my_feat_decoder(rendered_feat_bhwc)
            ...
            part_feat=F.interpolate(part_feat, size=gt_part_feat.shape[1:], mode="bilinear", align_corners=False)
            ...
            if viewpoint_cam.object is not None:
                gt_obj = viewpoint_cam.object.cuda().long()
                class_loss = cls_criterion(class_feat, gt_obj.unsqueeze(0)).squeeze().mean()
                loss = loss + class_loss
            # import pdb; pdb.set_trace()
            feat_loss = 0.5 * part_loss + 0.1 * dino_loss
