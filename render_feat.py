...
from scene import skip_feat_decoder,skip_feat_class_decoder
import cv2
...
def render_interpolating_trajectory(args, model_path, iteration, views, gaussians, pipeline, background, camera_slerp_list=None,
    ...
    if clip_feat or dino_feat:
        ...
        my_feat_decoder = skip_feat_class_decoder(gaussians._distill_features.shape[1]).cuda()
        ...
        for i in trange(step_size):
            ...
            if with_feat:
                ...
                if clip_feat or dino_feat:
                    ...
                    rendered_dino_feat, rendered_class_feat, rendered_part_feat = my_feat_decoder(rendered_feat)
                    rendered_dino_feat = rendered_dino_feat.squeeze(0)
                    rendered_class_feat = rendered_class_feat.squeeze(0)
                    ...
                    class_save_path = os.path.join(render_path, '{0}_{1}_step{2:05d}_class_feat'.format(idx, idx + 1, i) + ".npy")
                    ...
                    if text_query:
                        # rendered_clip_feat = rendered_part_feat / (rendered_part_feat.norm(dim=0, keepdim=True) + 1e-6)
