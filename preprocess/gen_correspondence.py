...
def run_source_img(loftr, folder, src_name, tgt_names=None, top_k=-1,
    ...
    {
    'no_filter':[
    {src_name:src_corr, tgt_name_i: tgt_corr_i}, 
    {src_name:src_corr, tgt_name_j: tgt_corr_j}, 
    ...
    ]
    }
    '''
...
def draw_corr(rgbA, rgbB, corrA, corrB, output_name):
    vis = np.concatenate([rgbA, rgbB], axis=1) # (h,w,c)
    # import pdb 
    # pdb.set_trace()
    radius = 2
...
def run_folder(loftr, folder, src_name, tgt_name, output_path, top_k):
    ...
    for src_name in pbar:
        ...
        results = run_source_img(loftr, folder, src_name, tgt_names, top_k=top_k, visualize=True)
